...
struct sym_data * decode_new_cap_encoded(uint32_t * string_index_ptr) {
  ...
  if (SID_symbol == 0) {
    ...
    if ((UTF8_compliant == 0) || (base_symbol < START_UTF8_2BYTE_SYMBOLS)) {
      ...
      if (base_symbol == 'C') {
        prior_is_cap = 1;
        /* The encoder sets sd[' '/'B'/'C'].type unconditionally (bit 2); the
           decoder must mirror that regardless of max_code_length, otherwise the
           sym_type ctx2 (2*(type&7)) for the following symbol diverges and the
           stream desyncs on short inputs (max_code_length < 14). */
